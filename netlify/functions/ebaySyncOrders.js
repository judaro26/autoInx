...
async function getEbayAccessToken() {
  ...
  const res = await axios.post(
    ...
    { headers: { 'Authorization': `Basic ${credentials}`, 'Content-Type': 'application/x-www-form-urlencoded' }, validateStatus: () => true }
  );
...
// ─── eBay Finances API (shipping labels only) ─────────────────────────────────
// NOTE: totalMarketplaceFee is already in the Fulfillment API response and is
// more reliable. We only call Finances API for SHIPPING_LABEL transactions,
// which represent eBay-purchased labels (not Shippo labels).
async function fetchEbayShippingLabelCost(accessToken, orderId) {
  try {
    ...
    if (res.status !== 200) {
      // 404 is common for older orders or accounts where orderId === legacyOrderId
      // Silently return 0 — fees come from the Fulfillment API instead
      console.log(`  ℹ️  Finances API ${res.status} for ${orderId} — using Fulfillment API fees`);
      return 0;
    }
    ...
    for (const tx of transactions) {
      if ((tx.transactionType || '').toUpperCase() === 'SHIPPING_LABEL') {
        shippingLabelCost += Math.abs(parseFloat(tx.amount?.value || 0));
        console.log(`  🏷️  eBay shipping label for ${orderId}: $${shippingLabelCost}`);
      }
    }
    return Math.round(shippingLabelCost * 100) / 100;
  } catch (err) {
    console.warn(`  ⚠️  Finances API exception for ${orderId}:`, err.message);
    return 0;
  }
}
...
function mapEbayOrder(o) {
  ...
  const trackingNumber = trackingFromFulfillments || step.shipmentTrackingNumber || null;
  ...
  // ── Amount: use pricingSummary.total (after discounts & promotions) ──────────
  // Do NOT use lineItemCost — that's the pre-discount subtotal.
  // pricingSummary.total is what the buyer actually paid (before eBay collected tax).
  const amount = Math.round(parseFloat(price.total?.value || line.lineItemCost?.value || 0) * 100) / 100;

  // ── Quantity: sum all line items ─────────────────────────────────────────────
  const quantity = (o.lineItems || []).reduce((sum, li) => sum + (li.quantity || 1), 0) || 1;
  ...
  const netPayoutFromEbay = Math.round(
    parseFloat(o.paymentSummary?.totalDueSeller?.value || 0) * 100
  ) / 100;

  console.log(`  📦 ${o.orderId}: amount=$${amount} qty=${quantity} fee=$${transactionCost} payout=$${netPayoutFromEbay}`);
  ...
  return {
    ...
    pricing: {
      quantity,
      shippingCharged,
      transactionCost,   // from Fulfillment API totalMarketplaceFee — accurate
      shippingLabelCost: 0,  // filled in below from Finances API if eBay label was purchased
      vendorCostPerUnit: null,
      netPayoutFromEbay, // bonus field — what eBay deposits to your account
    },
    meta: {
      sku: line.sku || null,
      shippingAddress,
  ...
  try {
    ...
    for (const o of ebayOrders) {
      try {
        ...
        // Finances API: only used for eBay-purchased shipping labels.
        // Fees come from totalMarketplaceFee in the Fulfillment API response above.
        const labelCost = await fetchEbayShippingLabelCost(ebayToken, mapped.externalOrderId);
        pricing.shippingLabelCost = labelCost;

        console.log(`  💰 ${mapped.externalOrderId}: fee=$${pricing.transactionCost} label=$${labelCost} payout=$${pricing.netPayoutFromEbay}`);
        ...
        if (forceResync) {
          diagnostics.push({
            ...
            transactionCost:  pricing.transactionCost,
            shippingLabelCost: labelCost,
            netPayoutFromEbay: pricing.netPayoutFromEbay,
          });
        }
        ...
        if (!savedId) {
          ...
          const coreUpdate       = { status: core.status, updatedAt: new Date().toISOString() };
          ...
          // Also update amount in case the order was discounted and we had the wrong value
          coreUpdate.amount = core.amount;

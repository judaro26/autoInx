...
<head>
    ...
    <style>
        ...
        /* START FIX: Corrected Toggle Switch CSS */
        .toggle-checkbox {
            position: absolute; /* FIX: Added absolute positioning for handle movement */
            top: 0;
            left: 0;
            margin: 0;
...
<body class="bg-gray-100 min-h-screen">
    ...
    <script type="module">
        ...
        function updateSubmitButtonState() {
            ...
            // Check if address is validated (data-validated="true") and if the admin cart has items (placeholder for cart logic)
            const isAddressValid = addressInput?.dataset.validated === 'true';
            // Placeholder: Check if cart has items (will always be false in this non-implemented version, but ready for future logic)
            const isCartNotEmpty = Object.keys(state.adminCart).length > 0; 
            
            // For now, only check address validity, as cart functions are placeholders
            if (submitButton) {
        ...
        function switchAdminTab(tab) {
            ...
            // FIX: Removed racing calls to fetchAdminConfig here. It is now handled 
            // by the deferred logic at the end of renderAdminPage.
            if (tab === 'users') {
        ...
        async function handleAddProduct(e) {
            ...
            if (!itemName || !itemDescription || !price || !catalogId) {
                showMessage('error', 'Please fill in all required product fields.');
                return;
            }
            ...
            try {
                ...
                showMessage('success', `Product "${itemName}" added successfully!`);
                form.reset();
        ...
        function handleDeleteCatalog(id, name) {
            ...
            showMessage('error', `Deleting category ${name} is not implemented in this version.`);
            resetTimer();
        }
        ...
        function handleDeleteProduct(id, name) {
            ...
            showMessage('error', `Deleting product ${name} is not implemented in this version.`);
            resetTimer();
        }
        ...
        function handleEditProduct(e) {
            e.preventDefault();
            showMessage('info', 'Product editing not implemented in this version.');
            resetTimer();
        }
        ...
        async function handleBulkUserAction() {
            ...
            showMessage('info', 'Bulk user actions are not implemented in this version.');
            resetTimer();
        }
        
        async function handleUserAction(action, uid, data = {}, successMsg, suppressRefresh = false) {
            // Placeholder: Individual User Action Logic
            showMessage('info', `Action ${action} for user ${uid} is not fully implemented.`);
        ...
        function promptForPasswordReset(uid) {
             // Placeholder: Password Reset Logic
              showMessage('info', 'Password reset is not implemented in this version.');
        }

        function deleteUser(uid, email) {
             // Placeholder: Delete User Logic
              showMessage('info', 'User deletion is not implemented in this version.');
        }

        function disableUser(uid, email, currentlyDisabled) {
             // Placeholder: Disable User Logic
              showMessage('info', 'User enable/disable is not implemented in this version.');
        }
        ...
        async function handleClearAnonymousUsers() {
            ...
            try {
                // FIX: Pass 'true' to force a token refresh and ensure admin claim is current
                ...
                do {
                    ...
                    if (pageToken) {
                        ...
                        showMessage('success', `Cleanup complete! Total anonymous users deleted: ${totalDeleted}.`, 8000);
                ...
                showMessage('error', `Cleanup failed: ${error.message}. Check Netlify logs.`, 8000);
            } finally {
        ...
        // --- ADMIN ORDER CREATION PLACEHOLDERS ---
        
        function handleAdminOrderSubmit(e) {
            e.preventDefault();
            showMessage('info', 'Manual order submission is not implemented in this version.');
        }

        function addAdminCartItem() {
            showMessage('info', 'Adding item to Admin Cart is not implemented.');
        }

        function removeAdminCartItem(itemId) {
            showMessage('info', 'Removing item from Admin Cart is not implemented.');
        }

        function updateAdminCartDisplay() {
            // Placeholder: Renders order cart for manual creation
            const el = document.getElementById('adminCartDisplay');
            if (el) el.innerHTML = '<p class="text-gray-500 italic">Add items from the selector above.</p>';
            const total = document.getElementById('adminCartTotal');
            if (total) total.textContent = formatPriceDisplay(0);

            updateSubmitButtonState(); // CRITICAL FIX 4: Call this on cart update
        }

        // --- GLOBAL HANDLERS (DEFINED OUTSIDE fetchAdminConfig for correct removeEventListener) ---
        ...
        async function fetchAdminConfig() {
            try {
                ...
                // CRITICAL FIX 1: Update global config state
                globalConfig = config;
                ...
                if (chatWidgetToggle && chatWidgetToggle.type === 'checkbox') {
                    // FIX: Remove/add listeners using the globally defined function names
                ...
                showMessage('error', `Failed to load configuration: ${error.message}`, 8000);
            }
        }
        ...
        async function handleUpdateConfig(e) {
            ...
            const updates = {
                // These already read the live DOM state (.checked), which is correct.
            ...
            try {
                ...
                showMessage('error', `Failed to save configuration: ${error.message}`, 8000);
            }
        }
        ...
        function renderAdminPage(userName) {
            ...
            if (state.activeTab === 'catalog' && !state.previewMode) renderProductListSection();
            if (state.activeTab === 'config' && !state.previewMode) { 
                // CRITICAL FIX 2: Defer config fetch until DOM elements are certainly available
